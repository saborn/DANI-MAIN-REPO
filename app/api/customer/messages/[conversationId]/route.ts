...
export async function GET(_req: Request, { params }: { params: { conversationId: string } }) {
  try {
    ...
    const messages = await DatabaseService.getMessagesBetween(CURRENT_CUSTOMER_ID, storeId)

    // Mark messages as read when customer views them
    await DatabaseService.markMessagesAsRead(CURRENT_CUSTOMER_ID, storeId)
...
export async function POST(req: Request, { params }: { params: { conversationId: string } }) {
  try {
    ...
    const newMessage = await DatabaseService.addMessage({
